...
public class TextMonsterPersister {
...
/*		Stage addStage = new Stage();
		Label idL = new Label("What is the Attack ID");
		TextField idT = new TextField("What is the Attack ID");
		TextField monT = new TextField("What is the Monsters Name");
		TextField dateT = new TextField("What is the date");
		TextField locationT = new TextField("What is the location");
		TextField nameT = new TextField("What is the Reporters Name");
		Button done = new Button("Done");
		done.setOnAction(event -> {
			String monster = monT.getText();
			String date = dateT.getText();
			String name = nameT.getText();
			String location = locationT.getText();
			String id = idT.getText();
			String monsterAttack = "Attack# " + id + ": " + monster + " attacked " + location + " on " + date + ". Reported by " + name + ".";
			monsterAttackList.add(monsterAttack);			
            addStage.close();
        });
		HBox addHBox = new HBox();
		addHBox.getChildren().addAll(idL,idT);
		VBox addVBox = new VBox();
		addVBox.getChildren().addAll(idT,monT, dateT,locationT,nameT, done);
		addStage.setTitle("My New Stage Title");
		Scene addScene = new Scene(addVBox, 600, 400);
